...
function renderLine(
  ...
  function openSpan(dec: MarkDecoration) {
    if (dec.widgetPlacement === 'replace') {
      // Skip the text and insert the widget instead
      if (dec.createWidget) {
  ...
  function closeSpan(dec: MarkDecoration) {
    if (dec.widgetPlacement === 'replace') return;
    html += '</span>';
  ...
  for (const evt of events) {
    ...
    if (evt.kind === 'open') {
      ...
      if (evt.decoration.widgetPlacement !== 'replace') {
