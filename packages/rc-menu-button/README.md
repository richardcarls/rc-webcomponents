...
| `placement`   | `placement`    | `AnchorPlacement`                         | `'block-end-start'` | Preferred placement of the popup relative to the trigger, in logical terms: `block-start*`/`block-end*` (above or below in horizontal text, beside the trigger in vertical text) or `inline-start*`/`inline-end*` (beside it along the reading direction), with an optional `-start`/`-end` edge alignment. When unset, switches to `'inline-end-start'` under vertical orientation. |
