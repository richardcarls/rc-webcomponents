...
function sanitizeRichMarkup(markup: string): string {
  const doc = new DOMParser().parseFromString(markup, 'text/html');

  for (const element of [...doc.body.querySelectorAll('*')]) {
    if (!SAFE_RICH_ELEMENTS.has(element.tagName)) {
    ...
    for (const attribute of [...element.attributes]) {
      const keepAttribute =
        (element.tagName === 'A' && attribute.name === 'href' && isSafeLinkHref(attribute.value)) ||
        (element.tagName === 'A' && attribute.name === 'title') ||
        (element.tagName === 'PRE' && attribute.name === 'data-lang');
...
export class RcMarkdownEditor extends LitElement {
  ...
  override firstUpdated() {
    ...
    if (!this._valueInitialized) {
      ...
      if ($textarea?.value) {
        this._defaultValue = $textarea.value;
      }
    }

    this._observer = new MutationObserver(() => {
      if (!this._ignoreRichMutations) {
        this._scheduleMarkdownSync();
      }
    });
    ...
    this._renderedMode = this.sourceMode ? 'source' : 'rich';
  ...
  override updated(changed: Map<string, unknown>) {
    if (changed.has('sourceMode') || changed.has('defaultSourceMode')) {
      if (this._firstUpdatedDone) {
        const nextMode: EditorMode = this.sourceMode ? 'source' : 'rich';

        if (nextMode !== this._renderedMode) {
          if (nextMode === 'source') {
        ...
        ? html`
            <div class="link-popover" role="dialog" aria-label="Link">
              <input
                ...
                aria-label="URL"
                @input=${(e: Event) => {
                  this._linkPopoverHref = (e.target as HTMLInputElement).value;
                }}
              ...
              <button
                ...
                class="link-popover-btn"
                title="Open link"
                ?disabled=${!this._linkPopoverHref}
                @click=${this._openLinkHref}
              >
                ↗
              </button>
              <button
  ...
  protected _renderMarkdown(markdown: string): string {
    ...
    if (langs.length === 0) {
      return sanitizeRichMarkup(markup);
    }
    ...
    langs.forEach((lang, i) => {
      if ($pres[i] && lang) {
        $pres[i].dataset['lang'] = lang;
      }
    });

    return sanitizeRichMarkup(doc.body.innerHTML);
  }
  ...
  protected _syncMarkdownFromRichView() {
    if (!this._$richView || this.sourceMode) {
      return;
    }
  ...
  protected _switchToSource() {
    if (!this._$richView || !this._$sourceEditor) {
      return;
    }
    ...
    this._$sourceEditor.usePlugin({
      mount: (api: RCTextareaPluginAPI) => {
        ...
        api.onCursorMove((start: number, end: number) => {
          ...
          const lang =
            formats.codeBlock && this._cachedTree
  ...
  protected _onSourceChange = (_e: Event) => {
    // The hidden source editor may emit while it initializes. Only the active
    // source surface is allowed to become the Markdown value; otherwise an
    // empty initialization event can erase controlled rich-view content.
    if (!this.sourceMode) {
      return;
    }
  ...
  protected _onToolbarAction = (e: CustomEvent<EditorToolbarActionDetail>) => {
    ...
    if (action === 'code-block-language') {
      if (this.sourceMode) {
        this._setSourceCodeBlockLanguage(codeLanguage ?? '');
      } else {
        this._setRichCodeBlockLanguage(codeLanguage ?? '');
      }
    ...
    if (action === 'heading' && headingLevel !== undefined) {
      if (this.sourceMode) {
        this._setSourceHeadingLevel(headingLevel);
      } else {
        document.execCommand('formatBlock', false, headingLevel ?? 'p');
      }
  ...
  protected _applyRichFormat(action: EditorToolbarAction) {
    if (this.readOnly || !this._$richView) {
      return;
    }
    ...
    const CHAR_SCOPE = new Set<EditorToolbarAction>([
      'bold',
      'italic',
      'underline',
      'strikethrough',
    ...
    if (CHAR_SCOPE.has(action)) {
      ...
      if (sel?.rangeCount) {
        this._$savedRange = sel.getRangeAt(0).cloneRange();
      }
    }
  ...
  protected _applySourceFormat(action: EditorToolbarAction) {
    ...
    if (wrap && api && api.selectionStart === api.selectionEnd) {
      ...
      if (wordInfo) {
        ...
        editor.value =
          api.value.slice(0, wordInfo.from) +
          pre +
          wordInfo.word +
          suf +
          api.value.slice(wordInfo.to);
    ...
    switch (action) {
      ...
      case 'link':
        ...
        break;
      case 'blockquote':
        this._toggleLinePrefix('> ');
        break;
      case 'bullet-list':
        this._toggleLinePrefix('- ');
        break;
      case 'ordered-list':
        this._toggleLinePrefix('1. ');
        break;
      case 'code-block':
        this._toggleSourceCodeFence();
        break;
    }
  }
  ...
  protected _setSourceHeadingLevel(level: HeadingLevel | null) {
    ...
    editor.value =
      ...
      prefix +
      stripped +
      value.slice(lineStart + (value.slice(lineStart).length - stripped.length));
  }
  ...
  protected _toggleSourceCodeFence() {
    ...
    editor.value =
      value.slice(0, lineStart) + fence + '\n' + selected + '\n' + fence + value.slice(lineEnd);
  }
  ...
  protected _onSelectionChange = () => {
    ...
    const sel =
      (this.shadowRoot as unknown as { getSelection?: () => Selection | null }).getSelection?.() ??
      window.getSelection();
    ...
    this._activeFormats = {
      ...
      link: !!$el?.closest('a'),
      heading: ($el?.closest('h1,h2,h3,h4,h5,h6')?.tagName.toLowerCase() ??
        null) as HeadingLevel | null,
  ...
  protected _pushToolbarState() {
    const $toolbar = this._$toolbar as
      | (HTMLElement & {
          activeBold?: boolean;
    ...
    this.dispatchEvent(
      new CustomEvent<ActiveFormats>('rc-formatting-change', {
        bubbles: true,
        composed: true,
  ...
  protected _expandRichSelectionToWord(): void {
    ...
    const modify = (
      sel as unknown as { modify?: (alter: string, dir: string, unit: string) => void }
    ).modify;
  ...
  protected _positionLinkPopover(): void {
    if (!this._$savedRange || !this._$linkPopover) {
      return;
    }
  ...
  protected _openLinkHref = () => {
    if (this._linkPopoverHref) {
      window.open(this._linkPopoverHref, '_blank', 'noopener,noreferrer');
    }
  };
  ...
  protected _dispatchChange(value: string) {
    this.dispatchEvent(
      new CustomEvent<{ value: string }>('rc-change', {
        bubbles: true,
        composed: true,
        detail: { value },
      }),
  ...
  protected _dispatchModeChange(mode: EditorMode) {
    this.dispatchEvent(
      new CustomEvent<{ mode: EditorMode }>('rc-mode-change', {
        bubbles: true,
        composed: true,
        detail: { mode },
      }),
