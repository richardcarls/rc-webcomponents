...
import './rc-textarea-v2.ts';
import type { RCTextareaV2 } from './rc-textarea-v2.ts';
import type { RCTextareaV2PluginAPI } from './types.ts';
import { getEditor, getLineNumbers, getSlottedTextarea, waitRender, simulatePaste } from './test-helpers.ts';

// ── Basic rendering and ARIA ──────────────────────────────────────────────────

describe('RCTextareaV2 — basic rendering', () => {
  test('renders the #editor contenteditable div', async () => {
    const screen = render(html`
      <rc-textarea-v2 data-testid="host" style="width: 400px; height: 200px;"></rc-textarea-v2>
    `);
    const host = screen.getByTestId('host').element() as RCTextareaV2;
  ...
  test('editor has role="textbox" and aria-multiline="true"', async () => {
    const screen = render(html`
      <rc-textarea-v2 data-testid="host"></rc-textarea-v2>
    `);
    const host = screen.getByTestId('host').element() as RCTextareaV2;
    await host.updateComplete;
...
describe('RCTextareaV2 — value', () => {
  ...
  test('setting the same value twice does not trigger a second render', async () => {
    ...
    expect(getEditor(host).querySelector('.v2-line')?.textContent).toBe('hello');
...
describe('RCTextareaV2 — readOnly', () => {
  test('readOnly=true sets contentEditable to false', async () => {
    const screen = render(html`
      <rc-textarea-v2 data-testid="host" read-only></rc-textarea-v2>
    `);
    const host = screen.getByTestId('host').element() as RCTextareaV2;
    await host.updateComplete;
...
describe('RCTextareaV2 — label', () => {
  ...
  test('label attribute sets aria-label at initial render', async () => {
    const screen = render(html`
      <rc-textarea-v2 data-testid="host" label="Code editor"></rc-textarea-v2>
    `);
    const host = screen.getByTestId('host').element() as RCTextareaV2;
    await host.updateComplete;
...
describe('RCTextareaV2 — lineNumbers', () => {
  ...
  test('lineNumbers=true: gutter gets one span per line', async () => {
    const screen = render(html`
      <rc-textarea-v2 data-testid="host" line-numbers></rc-textarea-v2>
    `);
    const host = screen.getByTestId('host').element() as RCTextareaV2;
    await host.updateComplete;
...
describe('RCTextareaV2 — slotted textarea', () => {
  test('slotted textarea is visually hidden', async () => {
    const screen = render(html`
      <rc-textarea-v2 data-testid="host">
        <textarea></textarea>
      </rc-textarea-v2>
    `);
    const host = screen.getByTestId('host').element() as RCTextareaV2;
...
describe('RCTextareaV2 — plugin API', () => {
  test('plugin.mount() is called with the plugin API', async () => {
    ...
    let mountedApi: RCTextareaV2PluginAPI | null = null;
    host.usePlugin({ mount(api) { mountedApi = api; } });
  ...
  test('plugin.destroy() is called on removePlugin()', async () => {
    ...
    let destroyed = false;
    host.usePlugin({ destroy() { destroyed = true; } });
    host.removePlugin();
  ...
  test('replacing a plugin calls destroy() on the old one', async () => {
    ...
    host.usePlugin({ destroy() { oldDestroyed = true; } });
  ...
  test('api.decorationsFromHtml() parses highlight.js-style spans', async () => {
    ...
    let parsedDecs: ReturnType<RCTextareaV2PluginAPI['decorationsFromHtml']> = [];
    host.usePlugin({
      mount(api) {
...
describe('RCTextareaV2 — pattern API', () => {
  ...
  test('addPattern() renders mark spans for matching text', async () => {
    ...
    expect(getEditor(host).querySelector('.hi-mark')!.textContent).toBe('hello');
  });
  ...
  test('pattern with formatting properties renders inline styles', async () => {
    ...
    host.addPattern({ pattern: /bold/g, className: 'bold-mark', bold: true, color: '#ff0000' });
    host.value = 'this is bold text';
    await waitRender();

    const span = getEditor(host).querySelector('.bold-mark') as HTMLElement | null;
    expect(span).not.toBeNull();
...
describe('RCTextareaV2 — events', () => {
  test('rc-textarea-v2-change fires on value setter', async () => {
    const screen = render(html`
      <rc-textarea-v2 data-testid="host"></rc-textarea-v2>
    `);
    const host = screen.getByTestId('host').element() as RCTextareaV2;
    ...
    host.addEventListener('rc-textarea-v2-change', (e) => events.push(e as CustomEvent));
  ...
  test('rc-textarea-v2-change bubbles and is composed', async () => {
    const screen = render(html`
      <rc-textarea-v2 data-testid="host"></rc-textarea-v2>
    `);
    const host = screen.getByTestId('host').element() as RCTextareaV2;
    ...
    const events: CustomEvent[] = [];
    document.addEventListener('rc-textarea-v2-change', (e) => events.push(e as CustomEvent));

    host.value = 'bubbling';

    document.removeEventListener('rc-textarea-v2-change', (e) => events.push(e as CustomEvent));
...
describe('RCTextareaV2 — decoration edge cases', () => {
...
describe('RCTextareaV2 — paste', () => {
  ...
  test('pasting multi-line text fires rc-textarea-v2-change with the full value', async () => {
    const screen = render(html`
      <rc-textarea-v2 data-testid="host"></rc-textarea-v2>
    `);
    const host = screen.getByTestId('host').element() as RCTextareaV2;
