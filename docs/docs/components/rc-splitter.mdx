...
```js
import '@rcarls/rc-webcomponents/rc-splitter/define';
```
...
Set `mode="fixed"` when the primary pane should be a clampable pixel width
while the secondary pane remains fluid. This is useful for canonical
list-detail layouts that start with a fixed list pane but still allow user
resize:

```html
<rc-splitter mode="fixed" min="360" max="412" value="360">
  <section>List</section>
  <section slot="secondary">Detail</section>
</rc-splitter>
```

The legacy `fixed` boolean still means “disable resizing” and is distinct from
`mode="fixed"`.

### Anchored settling

Set `snap-points` to ascending whitespace-separated values in the active mode's
units. Slow releases settle to the nearest point, while a swipe of at least
`swipe-velocity` (default `500` px/s) moves to the next point in that direction.
Omit `snap-points` to keep pointer resizing continuous.
...
Use `snapTo(index, 'animated')` to settle programmatically or
`snapTo(index, 'instant')` without animation. Animated settling uses
`--rc-splitter-snap-duration` (default `200ms`) and respects reduced motion.
