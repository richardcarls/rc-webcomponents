...
```js
import '@rcarls/rc-webcomponents/rc-bottom-sheet/define';
```
...
Without a theme, the authored handle uses a 32 by 4 pixel pill indicator,
centered inside a full-width, 48 pixel-high target.

The Substrate theme replaces that pill with the same three-dot visual language
as its splitter handle while preserving the full-width pointer target. Material
keeps the Material 3 pill indicator.
...
`rc-bottom-sheet-resize-start` fires before an interactive resize changes the
sheet geometry. Consumers can use it to prepare content that an expansion drag
will progressively reveal.
...
the sheet's block-end edge. Animated snaps emit only after the settle motion
finishes; an interrupted animation does not emit a stale target. This makes the
event a safe point for discrete content or accessibility-state changes.
...
Keep it as the dialog's first direct child and avoid adding dialog top padding
so its 48 pixel target reaches the sheet's top edge.
