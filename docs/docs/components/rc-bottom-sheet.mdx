...
Without a theme, the authored handle uses the same 32 by 4 pixel pill geometry
as the splitter-style drag indicator, centered inside a full-width, 48
pixel-high target.
...
easing function to customize the settle curve. The unthemed fallback is
