...
```js
import '@rcarls/rc-webcomponents/rc-fab/define';
```
...
Apply `rc-theme-material` to achieve the Material Design FAB style, or customize with
...
For `block-end-inline-start` also cover the inline-start edge (the left in LTR):

```css
rc-fab[position='block-end-inline-start'] {
...
Apply `rc-theme-material` for Material defaults. Size classes stay in CSS rather
...
Use `rc-fab--small` and `rc-fab--large` for Material size presets, or set
`--rc-fab-size` and `--rc-fab-radius` directly. For an action menu, use
[`rc-fab-menu`](/components/rc-fab-menu).
