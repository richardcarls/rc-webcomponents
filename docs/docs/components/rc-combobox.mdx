...
```js
import '@rcarls/rc-webcomponents/rc-combobox/define';
```
...
Set `popup-mode="dialog"` when the available viewport is too constrained for an anchored
listbox. This presents the current chips, search field, options, and a Done action in a native
modal dialog. Selection and allow-create changes are staged until Done; close or Escape discards
the draft. Done produces one committed selection change.
...
Applications can switch `popupMode` from viewport state. The mode is latched while a popup is
open so a resize does not replace an active interaction.
...
In dialog mode, create events are deferred until Done and run before the selection commit.
...
**Uncontrolled** (default): the component owns selection after mount. Use `<option selected>` or the `defaultValue` property for the initial value — `defaultValue` accepts `string | string[]` and has no HTML attribute form. Listen to `rc-select-change` to observe changes.
...
### Firefox for Android: `<label>`-wrapped multiple selects (worked around)

On Firefox for Android (GeckoView), tapping anywhere within an ancestor `<label>`'s bounds opens the native picker dialog for a wrapped `<select multiple>`, even though the select is hidden: "Geckoview and Fennec open select dropdown on tap of parent label; other browsers do not." [Bugzilla 1475723](https://bugzilla.mozilla.org/show_bug.cgi?id=1475723)
...
See the [rc-select Known Issues](./rc-select.mdx#known-issues) entry for consumer implications (pre-upgrade behavior, form submission, constraint validation).
