...
<AtAGlance
  ...
  native="Requires a direct child native canvas"
...
```js
import '@rcarls/rc-webcomponents/rc-virtual-canvas/define';
```
...
The native `<canvas>` must be a direct child of `rc-virtual-canvas`. Keeping it in light DOM
preserves the consumer's drawing surface and accessible fallback content before and after the
