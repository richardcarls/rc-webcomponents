...
import { infrastructurePackages } from './aggregate-packages.mjs';
...
for (const { directory, manifest } of packages) {
  ...
  // Per-component subpaths let consumers install only the aggregate and still
  // pay for one component. Generated by `yarn codegen:aggregate`.
  const subpath = `./${basename(directory)}`;

  if (!aggregateManifest.exports?.[subpath]) {
    errors.push(`${manifest.name}: missing aggregate subpath export ${subpath}`);
  }

  if (manifest.exports?.['./define'] && !aggregateManifest.exports?.[`${subpath}/define`]) {
    errors.push(`${manifest.name}: missing aggregate subpath export ${subpath}/define`);
  }
}
